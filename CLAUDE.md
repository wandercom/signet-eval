...
- **Optional function adapter**: qualified against Claude 2.1.280 early access; the installer also retains the compatible 2.1.274 build. Disabled by default, separate from legacy adapters, and refuses detected legacy coexistence. The CLI cannot attest host plugin activation. No all-log/transcript guarantee; historical raw data is not migrated automatically.
